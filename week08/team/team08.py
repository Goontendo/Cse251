...
# -----------------------------------------------------------------------------
def merge_sort_process(arr, size=0):
    # TODO - Add your code here to use threads.  Each time the merge algorithm does a recursive
    ...
    if len(arr) > 1:
        ...
        if size <= 10:
            # Sorting the first half
            t1 = mp.Process(target=merge_sort_process, args=(L, size))
            print("T1 created")

            # Sorting the second half
            t2 = mp.Process(target=merge_sort_process, args=(R, size))
            print("T2 created")
...
def main():
    ...
    for merge_function, desc in merges:
        ...
        arr = [random.randint(1, 10_100_000) for _ in range(1_000)]
